<?php
	require_once PRESENTATION_DIR . 'more_company_reviews.php';
	$company_review = new MoreCompanyReviews();
	$company_review->init();

	echo '
	<div class="row">
		<div class="col s12 m8 offset-m2 l8 offset-l2">
			<h5 class="center grey-text text-darken-2"><a href="'.$company_review->mLinkToCompany.'" class="red-text text-lighten-2">'.$company_review->mCompanyName.'</a></h5>
		</div>
	</div>';
	if ($company_review->mTotalReviews != 0) {
							...
							if ($company_review->mCustomerId != 0 && $company_review->mCustomerId == $company_review->mReviews[$i]['customer_id']) {
								...
								<span class="grey-text text-darken-2" style="position:absolute;right:0;">
									...
									<ul id="company_review_dropdown_'.$i.'" class="dropdown-content">
										<li><a href="#!" id="submit_EditReview_'.$company_review->mReviews[$i]['review_id'].'"><small><i class="far fa-edit"></i> Edit</small></a></li>
								...
								<script>
									$(document).ready(function() {
										...
										$(".company_review_content_'.$i.'").on("click", "#submit_EditReview_'.$company_review->mReviews[$i]['review_id'].'", function(event) {
											event.preventDefault();
											var review = "'.$company_review->mReviews[$i]['review'].'";
											$.post(
											"'.$company_review->mLinkToMoreCompanyReviews.'",
											{Edit_Review: "", ReviewId: '.$company_review->mReviews[$i]['review_id'].'},
											function(data) {
		...
		if (count($company_review->mPostListPages ) > 1) {
			echo '
			<div class="row">
				<div class="col s12">
					<ul class="pagination center">';
						for ($m = 0; $m < count($company_review->mPostListPages); $m++) {
							if ($company_review->mPage == ($m+1)) {
								echo '<li class="waves-effect active red lighten-2"><a href="'.$company_review->mPostListPages[$m].'">'.($m+1).'</a></li>';
							}
							else {
								echo '<li class="waves-effect"><a href="'.$company_review->mPostListPages[$m].'">'.($m+1).'</a></li>';
							}
						}
					echo '
					</ul>
	...
	if ($company_review->mEnableAddJobReviewForm) { 
		echo '
					<br>
					<form class="col s12" method="post" action="'.$company_review->mLinkToMoreCompanyReviews.'">
						<div class="row">
							<div class="input-field col s12 l8 offset-l2">
								<h6>&nbsp;&nbsp;Compose review:</h6>
								<textarea id="reviewJob" name="review" placeholder="What do you think about '.$company_review->mCompanyName.', '.$company_review->mReviewerName.'?" style="width:100%;height:150px;padding:12px 20px;box-sizing:border-box;border:2px solid #ccc;border-radius:4px;background-color:white;resize:none;"></textarea>
